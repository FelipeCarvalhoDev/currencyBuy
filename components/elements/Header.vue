<template>
  <nav class="fixed w-full p-6 bg-gray-100 text-header h-header flex shadow-md z-30 transition-all ease-in-out duration-700" :class="scrollY > 90 ? 'bg-opacity-50 backdrop-blur-sm' : ''">
    <div class="flex items-center justify-between max-w-screen-xl mx-auto w-full">
      ...
      <!-- Drawer Menu -->
      <aside class="p-5 transform top-0 -left-[1px] w-9/12 bg-white fixed h-full overflow-auto ease-in-out transition-all duration-300 z-30" :class="isOpen ? 'translate-x-0' : '-translate-x-full'">
