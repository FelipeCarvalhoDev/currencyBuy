...
export default defineEventHandler(async (event) => {
  ...
  const tokenResponse = await axios.post(`https://www.linkedin.com/oauth/v2/accessToken?grant_type=authorization_code&redirect_uri=http://localhost:3000/api/auth/callback/linkedin&client_id=77zcfgc0nm85sj&client_secret=WPL_AP1.Srwr3fmv7X8RIYaX.twvaww==&code=${code}`);
